const feature_json_path = './feature.json';
...
document.addEventListener('DOMContentLoaded', async () => {
    const data = await getFeature(feature_json_path);
    addProductsToList(data);
    const feature = document.querySelector('.feature_grid');

    data.forEach(element => {
        feature.insertAdjacentHTML('beforeend', `
    <div class="feature_item">
                <img class="feature_item_img" src="${element.img}" alt="photo1">
                ...
                <div class="feature_txt">
                    <h5 class="${element.h5}"</h5>
                    <p class="feature_item_p">${element.item_p}</p>
                    <h5 class="feature_price">$${element.price}</h5>
                </div>
            </div>
    `)
